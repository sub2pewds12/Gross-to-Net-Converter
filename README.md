...
This project provides a tool to calculate Vietnamese Net income from Gross income based on regulations applicable around April 2025. It includes:
...
```bash
git clone [https://github.com/sub2pewds12/Gross-to-Net-Converter](https://github.com/sub2pewds12/Gross-to-Net-Converter)
...
```bash
pip install uv
# Install runtime dependencies including those for Excel feature
uv pip install "streamlit>=1.20.0" "fastapi>=0.90.0" "uvicorn[standard]>=0.20.0" "pydantic>=2.0.0" "pandas>=1.5.0" "openpyxl>=3.0.0"
...
# Or just runtime dependencies (ensure pandas/openpyxl are listed in pyproject.toml's main deps):
# pip install .
...
### Option 1: Using Docker Compose (Recommended for Development & Local Testing)

This method runs both the API and the Web UI together in isolated containers using your local code (due to volume mounts). It's the simplest way to start for development if you have Docker installed and the source code cloned.
...
3.  **Run the application:** Execute the following command:
    ...
    ```
      ...
      * It will build the necessary Docker images the first time you run it (or if code/dependencies change) using the `--build` flag.
      * It then starts both the API and the Frontend UI services.
      * You will see logs from both services appear in your terminal.
4.  **Wait for Startup:** Wait a few moments until you see log messages indicating both servers (Uvicorn/Streamlit) are running, similar to:
      * `api-1     | INFO:     Uvicorn running on http://0.0.0.0:8000...`
      * `frontend-1|   URL: http://0.0.0.0:8501`
5.  **Access the Applications:** Open your web browser:
      * **Web UI:** Go to `http://localhost:8501`
      * **API Docs (Optional):** Go to `http://localhost:8000/docs`
6.  **Stop the Application:** When you are finished, go back to the terminal where the application is running and press `Ctrl+C`. Then, run the following command to clean up the containers and network:
    ```bash
    docker compose -f docker/docker-compose.yml down
    ```

### Option 2: Running Pre-Built Images from Docker Hub (For Users)

This option allows anyone with Docker installed to run the application using the images you pushed to Docker Hub (e.g., under your username `leanhkhoi1010`), without needing the source code.
...
1.  **Create `docker-compose-run.yml` file:** Create a *new file* in any directory on your computer and name it `docker-compose-run.yml`. Paste the following content into it:
    ...
    services:
      ...
      frontend:
        ...
        depends_on: # Wait for API service to start first (basic ordering)
          - api
        restart: unless-stopped # Optional: Restart policy
    ```

    *(Ensure the `image:` names exactly match your repositories on Docker Hub).*
...
### Option 3: Running Services Manually (Alternative - No Docker needed)
