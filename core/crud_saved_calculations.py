...
def get_saved_calculation(
    ...
    calculation = (
        db.query(SavedCalculationDB)
        .filter(SavedCalculationDB.id == calculation_id)
        .first()
    )
    if calculation:
...
def get_all_saved_calculations(
    ...
    """
    logger.debug(
        f"Attempting to retrieve all saved calculations with skip: {skip}, limit: {limit}"
    )
    calculations = (
        db.query(SavedCalculationDB)
        .order_by(SavedCalculationDB.created_at.desc())
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def create_saved_calculation(
    db: Session, calculation: SavedCalculationCreate
) -> SavedCalculationDB:
    """
    ...
    """
    logger.info(
        f"Attempting to create new saved calculation: {calculation.model_dump_json(exclude_none=True)}"
    )
    ...
    try:
        ...
        db.refresh(
            db_calculation
        )  # Refresh to get ID and default values like created_at
        logger.info(
            f"Successfully created saved calculation with ID: {db_calculation.id}"
        )
...
def update_saved_calculation(
    ...
    """
    logger.info(
        f"Attempting to update saved calculation ID {calculation_id} with data: {calculation_update.model_dump_json(exclude_none=True)}"
    )
    db_calculation = get_saved_calculation(db, calculation_id)

    if not db_calculation:
        logger.warning(
            f"No saved calculation found with ID {calculation_id} for update."
        )
        return None
    ...
    if not update_data:
        logger.info(
            f"No fields provided to update for calculation ID {calculation_id}."
        )
        return (
            db_calculation  # Return the existing object if no update data is provided
        )
    ...
    for key, value in update_data.items():
        if hasattr(db_calculation, key):
            ...
            logger.warning(
                f"Field '{key}' not found in SavedCalculationDB model during update for ID {calculation_id}."
            )
    ...
    try:
        ...
        db.rollback()
        logger.error(
            f"Error updating saved calculation ID {calculation_id} in DB: {str(e)}",
            exc_info=True,
        )
        raise


def delete_saved_calculation(
    db: Session, calculation_id: int
) -> Optional[SavedCalculationDB]:
    """
    ...
    if not db_calculation:
        logger.warning(
            f"No saved calculation found with ID {calculation_id} for deletion."
        )
        return None
    ...
    try:
        ...
        db.rollback()
        logger.error(
            f"Error deleting saved calculation ID {calculation_id} from DB: {str(e)}",
            exc_info=True,
        )
        raise
