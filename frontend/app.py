...
with tab3:
    ...
    if st.button(
        ...
        key="refresh_saved_calcs_button",
        use_container_width=True,
    ):
        ...
        if st.session_state.saved_calculations_error:
            ...
            st.success(
                f"Fetched {len(st.session_state.saved_calculations_data)} records."
            )
        st.session_state.initial_fetch_done_saved_calcs = (
            True  # Mark that refresh was attempted
        )
    ...
    if st.session_state.saved_calculations_data:
        ...
        not st.session_state.saved_calculations_error
    ):  # Only show this if no error and no data
        st.info(
            "Click 'Refresh Saved Calculations' to load data or if the list is empty."
        )
