...
router = APIRouter(
    prefix="/saved-calculations",
    tags=["Saved Calculations"]
)
...
async def create_new_saved_calculation_endpoint( # Renamed to avoid conflict if you import the function
    ...
    logger.info(f"Received request to create saved calculation: {calculation_data.model_dump_json(exclude_none=True)}")
    try:
        # Use the directly imported function
        created_calculation = create_saved_calculation(
            db=db, calculation=calculation_data
        )
        logger.info(f"Successfully created saved calculation with ID: {created_calculation.id}")
...
async def read_all_saved_calculations_endpoint( # Renamed
    ...
    limit: int = Query(100, ge=0, le=1000, description="Maximum number of records to return"),
    db: Session = Depends(database.get_db)
):
    logger.info(f"Received request to list saved calculations with skip={skip}, limit={limit}")
    try:
        calculations = get_all_saved_calculations(db, skip=skip, limit=limit)
        total_count = count_saved_calculations(db)
        logger.info(f"Retrieved {len(calculations)} saved calculations. Total count: {total_count}")
        return SavedCalculationListResponse(items=calculations, total=total_count, skip=skip, limit=limit)
    except Exception as e:
...
async def update_existing_saved_calculation_endpoint( # Renamed
    ...
    logger.info(f"Received request to update saved calculation ID {calculation_id} with data: {calculation_update_data.model_dump_json(exclude_none=True)}")
    ...
    if updated_calculation is None:
        logger.warning(f"Saved calculation with ID {calculation_id} not found for update.")
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Saved calculation with ID {calculation_id} not found"
        )
...
async def delete_existing_saved_calculation_endpoint( # Renamed
    ...
    logger.info(f"Received request to delete saved calculation with ID: {calculation_id}")
    deleted_calculation = delete_saved_calculation(db, calculation_id=calculation_id)
    if deleted_calculation is None:
        logger.warning(f"Saved calculation with ID {calculation_id} not found for deletion.")
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Saved calculation with ID {calculation_id} not found"
        )
